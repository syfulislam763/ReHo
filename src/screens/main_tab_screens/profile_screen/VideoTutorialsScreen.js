...
import { View, Text, TouchableOpacity, ScrollView, Image, Modal, ActivityIndicator, PanResponder } from 'react-native';
import { Clock, Play, X, Pause, Volume2, VolumeX, Maximize, SkipBack, SkipForward } from 'lucide-react-native';
...
const VideoTutorialsScreen = () => {
            ...
            if (video.videoUrl) {
              try {
                ...
                const { uri } = await VideoThumbnails.getThumbnailAsync(
                  video.videoUrl,
                  { time: 1000, quality: 0.5 } // Get thumbnail at 1 second mark
                );
  ...
  return (
    <ComponentWrapper bg_color="bg-[#5055ba]" title="Financial Videos">
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          <View className="flex-row flex-wrap justify-between">
            {filteredVideos.map((v) => (
              // Ensure video object has a unique key, using _id as per your existing code structure
      ...
      {/* Full Video Modal */}
      <Modal visible={!!selectedVideo} transparent animationType="fade" onRequestClose={handleVideoClose}>
            ...
            {status && status.isLoaded && (
              <View className="absolute bottom-16 left-0 right-0 px-6">
                ...
                <View className="flex-row items-center mb-4">
                  <Text className="text-white text-xs mr-2">{formatTime(status.positionMillis)}</Text>
                  <View
                ...
                <View className="flex-row justify-center items-center">
                  ...
                  <TouchableOpacity className="w-12 h-12 bg-white/20 rounded-full items-center justify-center" onPress={skipForward}>
